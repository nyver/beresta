...
- a Flutter Android application with local-first onboarding/unlock (account creation always succeeds locally first regardless of what happens next; an optional "Connect to a sync server?" prompt follows creation - Connect now opens the server sheet as soon as the shell mounts, Skip enters the shell directly - mirroring desktop's own post-create sync prompt) and a passphrase field carrying the platform-native `AutofillHints.newPassword`/`AutofillHints.password` hint depending on whether an account is being created or unlocked, so a password manager only offers to save or fill it in the situation that actually applies, a nested notebook hierarchy (create/rename/delete, move notes between notebooks), tag management (create/delete, assign/unassign per note, filter the note list by tag), virtualized notes with per-note deletion, a `flutter_quill` WYSIWYG body editor toolbar-restricted to match desktop's own Quill formatting set (bold/italic/strike/inline code, H1-H3, ordered/bullet list, blockquote, code block, link), a paste handler (`mobile/lib/paste_format.dart`, mirroring desktop's `pasteFormat.ts`) that degrades a pasted format outside that set down to plain text instead of silently dropping it later at export, and round-tripped through the same canonical Markdown projection as `GetNote`/`SaveNote` (`mobile/lib/markdown_delta.dart`, a Dart port of `core/sync/yjsadapter`'s Markdown parser/renderer kept byte-identical to it) - `GetNote` also returns a `base_revision` the editor carries forward and passes back into every `SaveNote` call (including after a revision restore), so a save merges the client's edit against that known ancestor into the note's live CRDT state instead of blindly replacing it and silently discarding a background sync merge the always-Markdown-only Android editor has no other way to have seen, attachments with an inline thumbnail/preview strip and per-attachment deletion, a default new-note title that clears when its title field receives focus, search, revisions, secure lifecycle handling, background synchronization, encrypted SAF backups, share capture, and a private quick-note widget;
...
The server sheet's synchronization section now mirrors desktop's unsafe-incoming-operation journal: an operation that fails authentication, decoding, or safe application is quarantined without advancing the durable cursor, and the sheet lists its sanitized operation ID and rejection reason with a Retry action that re-attempts verification from scratch (`core/mobileapi.Service.ListSyncQuarantine`/`RetryQuarantined`). The journal only appears when synchronization needs user action or an entry is present, so it never crowds the sheet in ordinary offline/pending/current states.
