...
|---|---|---|---|
| Attacker impersonates a registered device | Unauthorized sync, data injection, metadata access | Fresh single-use challenge bound to the server certificate fingerprint, device, and scope and signed by the device Ed25519 key; scoped hashed sessions with at most 24-hour TTL; refresh requires new proof | Challenge replay, cross-server proof, wrong-key, expired-session, and revoked-device tests |
...
| A shipped schema migration is later found to be wrong | Data loss or corruption not caused by a crash, so per-migration transaction rollback cannot help | Whole-file safety backup (WAL checkpoint, then write-temp/fsync/rename copy) taken before a pending migration runs on a pre-existing database; forward-fix recovery is a full-file restore, not per-migration down-SQL | Backup/restore round-trip and pending-vs-up-to-date backup-trigger tests |
| Backup archive is modified | Corrupt or attacker-controlled restore | Client authenticated encryption; strict server SHA-256 manifests; full pre-restore file and SQLite validation in temporary storage; atomic swap | Corrupt manifest/chunk and destroy/restore hash tests |
