...
// go build -ldflags="-X 'github.com/AnoRebel/go-sqlcipher.driverName=my-sqlite3'"
...
func (c *SQLiteConn) exec(ctx context.Context, query string, args []driver.NamedValue) (driver.Result, error) {
	...
	for {
		...
		if tail == "" {
			if res == nil {
				// https://github.com/AnoRebel/go-sqlcipher/issues/963
...
func (d *SQLiteDriver) Open(dsn string) (driver.Conn, error) {
	...
	// SQLCipher: PRAGMA key must be set before any other operations on encrypted databases
	if pragmaKey != "" {
	...
	// SQLCipher: PRAGMA cipher_page_size
	if pragmaCipherPageSize != -1 {
		query := fmt.Sprintf("PRAGMA cipher_page_size = %d;", pragmaCipherPageSize)
		if err := exec(query); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
	...
	// SQLCipher: PRAGMA cipher_compatibility (allows opening SQLCipher 3 databases)
	if pragmaCipherCompatibility != -1 {
		query := fmt.Sprintf("PRAGMA cipher_compatibility = %d;", pragmaCipherCompatibility)
		if err := exec(query); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
